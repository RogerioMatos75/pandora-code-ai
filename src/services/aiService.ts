...
import { CodeSuggestion, SecurityIssue, CodeExplanation, SecurityVulnerability, CodeGeneration } from '../types/AITypes';
...
export class AIService {
    ...
    private async initialize() {
        try {
            ...
            vscode.window.showErrorMessage('Falha ao inicializar o DeepSeek. Verifique se o modelo está instalado.');
        }
    }
    ...
    async getSuggestions(document: vscode.TextDocument, position: vscode.Position): Promise<CodeSuggestion[]> {
        try {
            ...
            vscode.window.showErrorMessage('Não consegui gerar sugestões. Tente novamente.');
            return [];
        }
    }
    ...
    async explainCode(code: string): Promise<CodeExplanation> {
        try {
            ...
            vscode.window.showErrorMessage('Não consegui explicar o código. Tente novamente.');
    ...
    async checkSecurity(code: string): Promise<SecurityIssue[]> {
        try {
            ...
            vscode.window.showErrorMessage('Não consegui verificar a segurança. Tente novamente.');
            return [];
        }
    }

    async analyzeCodeSecurity(document: vscode.TextDocument): Promise<SecurityVulnerability[]> {
        try {
    ...
    async generateCode(description: string, language: string): Promise<CodeGeneration> {
        try {
            const prompt = `
